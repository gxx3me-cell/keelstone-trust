...
import { Vault, ArrowRight, EnvelopeSimple } from '@phosphor-icons/react'
    ...
    <div ref={rootRef} style={{ minHeight: '100vh', background: C.ink, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative', overflow: 'hidden' }}>
      ...
      <div data-anim style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 440 }}>
        ...
        <Link to="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, textDecoration: 'none', marginBottom: 36 }}>
          <div style={{ width: 36, height: 36, borderRadius: 9, background: 'rgba(255,255,255,.1)', border: '1px solid rgba(255,255,255,.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Vault size={18} color="#fff" weight="duotone" />
          </div>
          <span style={{ fontFamily: serif, fontSize: 20, color: '#fff' }}>Lumen</span>
        </Link>
          ...
          {done ? (
            <div style={{ textAlign: 'center', padding: '16px 0' }}>
              ...
              <p style={{ fontSize: 14.5, color: C.muted, lineHeight: 1.65, margin: '0 0 28px' }}>
                If <strong style={{ color: C.ink }}>{email}</strong> is registered with Lumen, you'll receive a password reset link shortly.
              </p>
            ...
            <>
              ...
              <p style={{ fontSize: 14.5, color: C.muted, margin: '0 0 28px', lineHeight: 1.6 }}>
                Enter the email address linked to your Lumen account and we'll send you a reset link.
              </p>
